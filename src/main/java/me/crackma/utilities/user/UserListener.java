...
public class UserListener implements Listener {
  ...
  public UserListener(UtilitiesPlugin plugin) {
    this.plugin = plugin;
    joinMessage = "";
    plugin.getConfig().getStringList("joinMessage").forEach(line -> joinMessage = joinMessage + ChatColor.translateAlternateColorCodes('&', line) + "\n");
  ...
  public void onLogin(PlayerLoginEvent event) {
    ...
    user.setPermissionAttachment(player.addAttachment(plugin));
    ...
    if (player.hasPermission("utilities.vanishonlogin")) {
  ...
  public void onChat(AsyncPlayerChatEvent event) {
    ...
    if (plugin.isChatMuted() && !player.hasPermission("utilities.chatmutebypass")) {
      player.sendMessage("§cYou cannot talk while the chat is muted.");
      return;
    }
